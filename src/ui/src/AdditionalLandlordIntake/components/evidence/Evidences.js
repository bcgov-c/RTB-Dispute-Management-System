...
export default Marionette.View.extend({
  ...
  initialize(options) {
    this.mergeOptions(options, ['hideHidden']);
  },

  onRender() {
    const viewOptions = Object.assign({ collection: this.collection }, this.hideHidden ? { filter(child) { return !child.get('isHidden'); } } : {});
    this.showChildView('collectionRegion', new EvidenceCollectionView(viewOptions));
  },
