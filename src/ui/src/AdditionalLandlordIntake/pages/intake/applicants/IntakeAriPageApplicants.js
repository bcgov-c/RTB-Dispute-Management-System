...
import template from './IntakeAriPageApplicants_template.tpl';
...
export default PageView.extend({
  ...
  createPageItems() {
    ...
    const applicantCountValue = this.getPageItem('applicantCount').getModel().getData();
    const applicantDropdown = new DropdownModel(
      Object.assign({
        ...
        value: applicantCountValue ? Number(applicantCountValue) : null
      }, dropdownOptions)
    );
    ...
    const assistantCountValue = this.getPageItem('assistantCount').getModel().getData();
    ...
    const applicants = participantsChannel.request('get:applicants');
    this.intakeApplicants = new IntakeParticipantCollection(applicants.map(function(applicant) {
      return { participantModel: applicant, noPackageProvision: true, useMailAddressValidation: true, useAddressValidation: true, isOptional: false };
    }), { participantCollection: applicants, noPackageProvision: true, useMailAddressValidation: true, useAddressValidation: true, isOptional: false });
