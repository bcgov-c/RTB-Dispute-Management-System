...
const UpdateNoticeServiceReceipt = PageView.extend({
  ...
  clickCompleteService() {
    ...
    Backbone.history.navigate('update/notice/service', { trigger: true, replace: true });
  },
  ...
  initialize() {
    ...
    this.noticeServiceModel = this.receiptData.noticeServiceModel || new Backbone.Model();    this.submittedNoticeFiles = this.receiptData.submittedNoticeFiles || [];
  ...
  template() {
    ...
    const proofOfServiceWarningJsx = () => {
      ...
      return <p className="error-block warning"> You have not completed providing proof of service information for this dispute.  The Residential Tenancy Branch will not proceed with your hearing until this proof of service is completed.  To complete your proof of service now, click 'Complete Proof of Service'.</p>
    };
  ...
  receiptPageHtml() {
    ...
    const service_comment = this.noticeServiceModel.get('service_comment');
    ...
    const receiptData = [
      ...
      ...(service_method ? [{ label: 'Service method', value: Formatter.toNoticeMethodDisplay(service_method) }] : []),
      ...(service_date ? [{ label: 'Date served', value: Formatter.toDateDisplay(service_date) }] : []),
      ...(this.submittedNoticeFiles && this.submittedNoticeFiles.getUploaded() ? [{ label: 'Files submitted', value: Formatter.toUploadedFilesDisplay(this.submittedNoticeFiles) }] : []),
      ...(service_comment ? [{ label: 'Service comment', value: service_comment }] : [])
    ];
...
_.extend(UpdateNoticeServiceReceipt.prototype, ViewJSXMixin);
export default UpdateNoticeServiceReceipt;
