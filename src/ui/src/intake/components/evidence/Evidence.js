...
export default Marionette.View.extend({
  ...
  initialize(options) {
    this.mergeOptions(options, ['enableBigTrialIntervention', 'fileType']);
    ...
    this.stopListening(this.model, 'show:upload', this.showFileUploadModalWithTrialCheck, this);
    this.listenTo(this.model, 'show:upload', this.showFileUploadModalWithTrialCheck, this);
    ...
    if (value) {
      ...
      this.model.save(this.model.getApiChangesOnly())
        .then(() => {
          ...
          if (String(value) === '100') {
            this.showFileUploadModalWithTrialCheck();
          }
          this.render();
  ...
  showFileUploadModalWithTrialCheck() {
    if (!this.enableBigTrialIntervention) return this.showFileUploadModal();
    this.showTrialIntervention().then(() => this.showFileUploadModal());
  },
  ...
  showTrialIntervention() {
    ...
    const isDirectRequestSelected = claims.find(c => directRequestCodes.includes(c.get('claim_code')));
    if (isDirectRequestSelected || !TrialLogic_BIGEvidence.canViewIntakeEvidenceNudgeInterventions(dispute, primaryApplicant)) return Promise.resolve();
