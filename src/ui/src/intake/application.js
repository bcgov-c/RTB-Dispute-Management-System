...
import DisputeListPageView from './pages/dispute-list/DisputeListPage';
...
const AppModel = Backbone.Model.extend({
  ...
  refreshSessionProgress() {
    // NOTE: Disable refreshing routing.  Force Next
    return;
  },
  ...
  validateIncompleteStep(step_number) {
    ...
    const progressCheckFns = {
      ...
      5: this.isStepFiveIncomplete,
      6: this.isStepSixIncomplete
    };
  ...
  isStepThreeComplete() {
    // For Applcant Options to be complete, there must be a primary applicant
   ...
   * @returns {QuestionModel} - the QuesitonModel object that was created.
   */
  ...
  checkAndCreateRequiredIntakeQuestions() {
    const required_questions = configChannel.request('get', 'intake_questions');
    const dispute = disputeChannel.request('get');
...
const _webpackSetupHotModuleReloadHandlers = () => {
  ...
  module.hot.accept('./pages/intake/Intake', () => {
