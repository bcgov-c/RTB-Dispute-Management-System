...
export default NoticePreview.extend({
  ...
  initialize(options) {
    ...
    this.templateData = _.extend({}, this.templateData, {
        ...
        noticeTitleDisplay: this.isDisputeCreatedAriC ? (this.isPrelim ? ARI_C_PRELIM_NOTICE_TITLE : ARI_C_NOTICE_TITLE) : ARI_E_NOTICE_TITLE,
      },
      this.matchingUnit ? {
  ...
  getExpenseIssueHtml() {
    ...
    this.disputeClaim.getAllRemedies().forEach(remedyModel => {
      ...
      const eligibleRI = associatedCost && calculateCostRentIncreaseFn(remedyModel.getAmount(), associatedCost.getUnitIds().length);
  ...
  onRender() {
    ...
    // Some overrides only apply to ARI-C or ARI-E disputes
    if (this.isDisputeCreatedAriC) {
      ...
      if (this.isPrelim) {
        ...
        this.$('#Participatoryservice-instructions tr:nth-child(2) td').html(`
          <p class="text_content">A prehearing conference has been scheduled. The purpose of the prehearing conference is to address <b>procedural matters</b> and not the merits of the landlord's application for rent increase which is attached below. <b>No evidence will be considered at this conference.</b>
          </p>
          ...
          <ul class="bullet_list">
            ...
            <li>either side requires a Court Reporter to attend (Rule of Procedure 6.12)</li>
        ...
        this.$('.respondent-access-code').hide();
      }
    } else { // ARI-E only
