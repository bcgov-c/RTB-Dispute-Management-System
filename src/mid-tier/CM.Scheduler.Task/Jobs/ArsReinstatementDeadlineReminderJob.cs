...
namespace CM.Scheduler.Task.Jobs
{
    ...
    public class ArsReinstatementDeadlineReminderJob : JobBase
    {
        private const string Identity = "ars-reinstatement-deadline-reminder-job";

        public ArsReinstatementDeadlineReminderJob(IServiceScopeFactory serviceScopeFactory)
        {
        ...
        public override async System.Threading.Tasks.Task Execute(IJobExecutionContext context)
        {
            try
            {
                ...
                await service.ArsReinstatementDeadlineReminderNotifications();
                context.Result = true;
